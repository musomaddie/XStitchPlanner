...
SC_A = PatternCell("a", "310", [], "")
SC_B = PatternCell("b", "550", [], "")
SC_C = PatternCell("c", "666", [], "")
...
def test_init(stitcher, starting_corner):
    ...
    assert stitcher.stitched_pattern == [[StitchingCell(cell) for cell in row] for row in PATTERN]
    assert stitcher.starting_corner == starting_corner

    # Due to test set up the values propogate down
    if starting_corner == TOP_LEFT:
        assert stitcher.stitched_pattern[0][0].to_start_with
        assert not stitcher.stitched_pattern[0][4].to_start_with
        assert not stitcher.stitched_pattern[4][0].to_start_with
        assert not stitcher.stitched_pattern[4][4].to_start_with
    if starting_corner == TOP_RIGHT:
        assert stitcher.stitched_pattern[0][4].to_start_with
        assert not stitcher.stitched_pattern[4][0].to_start_with
        assert not stitcher.stitched_pattern[4][4].to_start_with
    if starting_corner == BOTTOM_LEFT:
        assert stitcher.stitched_pattern[4][0].to_start_with
        assert not stitcher.stitched_pattern[4][4].to_start_with
    if starting_corner == BOTTOM_RIGHT:
        assert stitcher.stitched_pattern[4][4].to_start_with
