...
@pytest.mark.parametrize(
    ("cur_x", "cur_y", "expected_page_height", "height", "width", "overlap", "result_pattern",
     "result_x", "result_y", "result_page_height"),
    # Simple example
    [(0, 0, 5, 5, 10, 0, EXAMPLE_ROWS_BASIC, 0, 5, 5),
     ...
     # With some overlap
     (10, 0, 5, 5, 18, 2, [row[2:] for row in EXAMPLE_ROWS_BASIC], 0, 5, 5)]
)
def test_extract_pattern_from_this_page(
    ...
    actual_rx, actual_ry, actual_rph = extractor._extract_from_this_page(
        0, EXAMPLE_ROWS_BASIC, cur_x, cur_y, expected_page_height, height, width, overlap, False)
...
@pytest.mark.parametrize(
    ...
    [(0, 0, 5, 5, 10, EXAMPLE_ROWS_BASIC + ["1", "2", "3"], s.pattern_uneven_width(1)),
     (2, 2, 7, 5, 10, EXAMPLE_ROWS_BASIC, s.pattern_uneven_height(1, 5, 7, 3)),
     (2, 2, 5, 7, 10, EXAMPLE_ROWS_BASIC, s.pattern_size_too_big(1, 10, 5, 12, 7, 10, 7))]
)
...
@pytest.mark.parametrize(
    ...
    [(10, 5, 0, 0, EXAMPLE_ROWS_BASIC), (10, 10, 0, 1, EXAMPLE_ROWS_BASIC + EXAMPLE_ROWS_BASIC)]
)
def test_extract_pattern_given_pages_valid(
        extractor, width, height, start_page, end_page, expected_pattern):
    extractor.extract_pattern_given_pages(fake_rows_fn, width, height, start_page, end_page)
    assert extractor.pattern == expected_pattern


@pytest.mark.parametrize(
    ("width", "height", "expected_error_message"),
    [(12, 5, s.pattern_wrong_size("wide", 10, 12)), (10, 7, s.pattern_wrong_size("high", 5, 7))]
)
def test_extract_pattern_give_pages_invalid(extractor, width, height, expected_error_message):
    extractor.pdf.pages = ["example"]
    with pytest.raises(PatternFormatError) as e:
        extractor.extract_pattern_given_pages(fake_rows_fn, width, height)
    assert str(e.value) == expected_error_message
