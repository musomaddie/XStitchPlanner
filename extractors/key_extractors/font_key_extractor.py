...
class FontKeyExtractor(KeyExtractor):
    """ A class for extracting the key when the PDF can be accessed via the text fields.
    ...
    def extract_key(
        ...
        # Set up
        first_page, last_page = determine_pages(key_start_page_idx, key_end_page_idx)
        ...
        for key_page_idx in range(first_page, last_page + 1):
            ...
            self.key.extend(self._extract_key_from_page(self.pdf.pages[key_page_idx],
    ...
    def _extract_key_from_page(
            ...
            key_page: Page,
            is_first_page: bool,
            verbose: bool = False) -> list[Thread]:
