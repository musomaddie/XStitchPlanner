...
Arguments:
  PDF           input pdf path
  STARTPAGE     page number of the first page containing the key. [default: 1]
  ENDPAGE       page number of the last page containing the key. If blank only the page specified
                by STARTPAGE will be used for the key.

Options:
  -h -help              show this help message and exit
  -v --verbose          print status messages
  -m MODE --mode=MODE   extract mode can either be "font" or "shape". [default: font].
  MODE:
    font:   The default extraction mode if MODE is not given. Identify symbols in the key by
            extracting the text from the PDF. A PDF can typically be read using font if you can
            copy the symbols from it by selecting them.
    shape:  Extract symbols in the key by attempting to identify and then match reoccurring lines
            and shapes. These identifiers are then matched up with arbitrary symbols for
            displaying. These identifiers may bear no resemblance to the original symbols.
"""
import pdfplumber
...
def extract_key_from_pdf(
    ...
    Raises:
        pdfminer.pdfparser.PDFSyntaxError: if the file found at pdf_name does exist,
            but it isn't a PDF.
...
if __name__ == "__main__":
    ...
    args = docopt(__doc__)
    extract_key_from_pdf(
        args["PDF"],
        ExtractorMode.from_string(args["--mode"]),
        start_page_idx=_subtract_one(_make_int(args["STARTPAGE"])),
        end_page_idx=_subtract_one(_make_int(args["ENDPAGE"])),
